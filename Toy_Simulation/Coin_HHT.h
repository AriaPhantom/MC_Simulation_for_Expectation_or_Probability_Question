...
void Coin_HHT(long times, double p)//p: Probability of head
{
	...
	for (int i = 0; i < times; i++)
	{
		...
		for (int n = 0; n < 10000; n++)
		{
			...
			if (n >= 2)
			{
				if (Coin_Store[n] == 0 && Coin_Store[n - 1] == 1 && Coin_Store[n - 2] == 1)
				{
					for (int q = 2; q <= n; q++)
					{
						if (Coin_Store[q] == 1 && Coin_Store[q - 1] == 0 && Coin_Store[q - 2] == 1)
						{
